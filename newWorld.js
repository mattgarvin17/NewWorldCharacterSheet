			...
			['might', 'agility', 'resolve', 'intellect', 'focus', 'charisma', 'devotion', 'fortune'].forEach(attr => {
			...
			['might', 'agility', 'resolve', 'intellect', 'focus', 'charisma', 'devotion', 'fortune'].forEach(attr => {
				on(`change:${attr}`, function() {
					...
					(attr === "might") ? update_weight(): false;
					(attr === "agility") ? update_initiative(): false;
					(attr === "intellect") ? update_initiative(): false;
				});
			});
			...
			['might', 'agility', 'resolve', 'intellect', 'focus', 'charisma', 'devotion', 'fortune'].forEach(attr => {
				on(`change:${attr}_mod`, function() {
					...
					switch (`${attr}`) {
						case "might":
							update_skills(["athletics", "grip"]);
							break;
						case "agility":
							update_skills(["acrobatics", "legerdemain", "stealth"]);
							update_ac();
							update_initiative();
							break;
                        case "resolve":
							update_skills(["endurance", "tolerance"]);
							break;
						case "intellect":
							update_skills(["arcana","civics","culture","deduction", "history", "mathematics", "medicine", "nature", "religion", "science", "technology", "vice", "warfare"]);
							update_initiative();
							break;
						case "focus":
							update_skills(["exploration", "insight", "perception"]);
							break;
						case "charisma":
							update_skills(["deception", "domestication", "intimidation", "performance", "persuasion"]);
							break;
                        case "devotion":
                            update_skills(["adamance"]);
                            break;
						default:
							false;
					}
			...
			['acrobatics', 'adamance', 'arcana', 'athletics', 'civics', 'culture', 'deception', 'deduction', 'domestication', 'endurance', 'exploration', 'grip', 'history', 'insight', 'intimidation', 'legerdemain', 'mathematics', 'medicine', 'nature', 'perception', 'performance', 'persuasion', 'religion', 'science', 'stealth', 'technology', 'tolerance', 'vice', 'warfare'].forEach(attr => {
			...
			on("change:kine change:subkine", function(eventinfo) {
				update_kine_display();
			});

			on("change:drop_category", function(eventinfo) {
				if (eventinfo.newValue === "Monsters") {
					getAttrs(["class", "kine", "speed", "hp"], function(v) {
						if (v["class"] != "" || v["kine"] != "" || v["speed"] != "" || v["hp"] != "") {
							setAttrs({
			...
			var update_all_saves = function() {
				...
				update_save("agility");
				update_save("resolve");
				update_save("intellect");
				update_save("focus");
				update_save("charisma");
                update_save("devotion");
                update_save("fortune");
				update_save("death");
			};
			...
			var update_all_ability_checks = function() {
				update_initiative();
				update_skills(["acrobatics", "adamance", "arcana", "athletics", "civics", "culture", "deception", "deduction", "domestication", "endurance", "exploration", "grip", "history", "insight", "intimidation", "legerdemain", "mathematics", "medicine", "nature", "perception", "performance", "persuasion", "religion", "science", "stealth", "technology", "tolerance", "vice", "warfare"]);
			};

			var update_skills = function(skills_array) {
				var attrs_to_get = ["pb", "pb_type", "jack_of_all_trades", "jack", "might_mod", "agility_mod", "resolve_mod", "intellect_mod", "focus_mod", "charisma_mod", "devotion_mod", "fortune_mod"];
				var update = {};
				var callbacks = [];
			...
			var do_update_tool = function(tool_array) {
				var tool_attribs = ["pb", "pb_type", "jack", "might_mod", "agility_mod", "resolve_mod", "intellect_mod", "focus_mod", "charisma_mod", "devotion_mod", "fortune_mod"];
				var update = {};
				...
				getAttrs(tool_attribs, function(v) {
					_.each(tool_array, function(tool_id) {
						...
						if (v["repeating_tool_" + tool_id + "_toolattr_base"] && v["repeating_tool_" + tool_id + "_toolattr_base"].substring(0, 2) === "?{") {
							...
							var mod = "?{Attribute?|Might,@{might_mod}|Agility,@{agility_mod}|Resolve,@{resolve_mod}|Intellect,@{intellect_mod}|Focus,@{focus_mod}|Charisma,@{charisma_mod}|Devotion,@{devotion_mod}|Fortune,@{fortune_mod}}";
			...
			var processDrop = function(page, currentData, repeating, looped) {
				...
				var modStringToAttrib = function(modString) {
					var finalAttrib = "";
					if (modString == "FIN") {
						if (parseInt(currentData.might_base) > parseInt(currentData.agility_base)) {
							finalAttrib = "@{might_mod}";
						} else {
						...
						switch (modString) {
							...
							case "AGI":
								...
								break;
							case "RES":
								finalAttrib = "@{resolve_mod}";
								break;
							case "INT":
								finalAttrib = "@{intellect_mod}";
								break;
							case "FOC":
								finalAttrib = "@{focus_mod}";
								break;
							case "CHA":
								finalAttrib = "@{charisma_mod}";
								break;
                            case "DEV":
								finalAttrib = "@{devotion_mod}";
								break;
                            case "FOR":
								finalAttrib = "@{fortune_mod}";
								break;
						}
					}
					return finalAttrib;
				...
				var parseValues = function(uses_string) {
					var attribs = ["might", "agility", "resolve", "intellect", "focus", "charisma", "devotion", "fortune"];
				...
				if (category === "Monsters") {
					...
					update["might_base"] = page.data["MIG"] ? page.data["MIG"] : "";
					update["agility_base"] = page.data["AGI"] ? page.data["AGI"] : "";
					update["resolve_base"] = page.data["RES"] ? page.data["RES"] : "";
					update["intellect_base"] = page.data["INT"] ? page.data["INT"] : "";
					update["focus_base"] = page.data["FOC"] ? page.data["FOC"] : "";
					update["charisma_base"] = page.data["CHA"] ? page.data["CHA"] : "";
                    update["devotion_base"] = page.data["DEV"] ? page.data["DEV"] : "";
                    update["fortune_base"] = page.data["FOR"] ? page.data["FOR"] : "";
					callbacks.push(function() {
						update_attr("might");
					});
					callbacks.push(function() {
					...
					update["npc_mig_save_base"] = "";
					update["npc_agi_save_base"] = "";
					update["npc_res_save_base"] = "";
					update["npc_int_save_base"] = "";
					update["npc_foc_save_base"] = "";
					update["npc_cha_save_base"] = "";
                    update["npc_dev_save_base"] = "";
                    update["npc_for_save_base"] = "";
					...
					update["npc_adamance_base"] = "";
					update["npc_arcana_base"] = "";
					update["npc_athletics_base"] = "";
                    update["npc_civics_base"] = "";
                    update["npc_culture_base"] = "";
					update["npc_deception_base"] = "";
                    update["npc_deduction_base"] = "";
                    update["npc_domestication_base"] = "";
                    update["npc_endurance_base"] = "";
                    update["npc_exploration_base"] = "";
					update["npc_history_base"] = "";
					update["npc_insight_base"] = "";
					update["npc_intimidation_base"] = "";
					update["npc_legerdemain_base"] = "";
                    update["npc_mathematics_base"] = "";
					...
					update["npc_science_base"] = "";
					update["npc_stealth_base"] = "";
					update["npc_technology_base"] = "";
                    update["npc_tolerance_base"] = "";
                    update["npc_vice_base"] = "";
                    update["npc_warfare_base"] = "";
				...
				if (category === "Proficiencies") {
					...
					if (type.toLowerCase() === "language" || type.toLowerCase() === "armor" ||
						...
						if (!existing.base) update["repeating_tool_" + newrowid + "_toolattr_base"] = "?{Attribute?|Might,@{might_mod}|Agility,@{agility_mod}|Resolve,@{resolve_mod}|Intellect,@{intellect_mod}|Focus,@{focus_mod}|Charisma,@{charisma_mod}|Devotion,@{devotion_mod}|Fortune,@{fortune_mod}}";
				...
				if (category === "Kines" || category === "Subkines") {
					update["tab"] = "core";
					if (category === "Kines") {
						update["kine"] = page.name;
						if (page.name == "Halfling") {
							update["halflingluck_flag"] = "1";
						}
					} else {
						update["subkine"] = page.name;
					};
					if (page.data["Speed"]) {
					...
					if (!looped) {
						callbacks.push(update_kine_display);
					}
				};
			...
			var check_itemmodifiers = function(modifiers, previousValue) {
				...
				_.each(mods, function(mod) {
					...
					if (mod.indexOf("might save") > -1) {
						update_save("might");
					} else if (mod.indexOf("might") > -1) {
						update_attr("might");
					};
					if (mod.indexOf("agility save") > -1) {
						update_save("agility");
					} else if (mod.indexOf("agility") > -1) {
						update_attr("agility");
					};
					if (mod.indexOf("resolve save") > -1) {
						update_save("resolve");
					} else if (mod.indexOf("resolve") > -1) {
						update_attr("resolve");
					};
					if (mod.indexOf("intellect save") > -1) {
						update_save("intellect");
					} else if (mod.indexOf("intellect") > -1) {
						update_attr("intellect");
					};
					if (mod.indexOf("focus save") > -1) {
						update_save("focus");
					} else if (mod.indexOf("focus") > -1) {
						update_attr("focus");
					};
					...
					if (mod.indexOf("devotion save") > -1) {
						update_save("devotion");
					} else if (mod.indexOf("devotion") > -1) {
						update_attr("devotion");
					};
					if (mod.indexOf("fortune save") > -1) {
						update_save("fortune");
					} else if (mod.indexOf("fortune") > -1) {
						update_attr("fortune");
					};
					...
					if (mod.indexOf("civics") > -1) {
						update_skills(["civics"]);
					};
					if (mod.indexOf("culture") > -1) {
						update_skills(["culture"]);
					};
					if (mod.indexOf("deception") > -1) {
						update_skills(["deception"]);
					};
					if (mod.indexOf("deduction") > -1) {
						update_skills(["deduction"]);
					};
					if (mod.indexOf("domestication") > -1) {
						update_skills(["domestication"]);
					};
					if (mod.indexOf("endurance") > -1) {
						update_skills(["endurance"]);
					};
					if (mod.indexOf("exploration") > -1) {
						update_skills(["exploration"]);
					};
					if (mod.indexOf("grip") > -1) {
						update_skills(["grip"]);
					};
					...
					if (mod.indexOf("legerdemain") > -1) {
						update_skills(["investigation"]);
					};
					if (mod.indexOf("mathematics") > -1) {
						update_skills(["mathematics"]);
					};
					...
					if (mod.indexOf("science") > -1) {
						update_skills(["science"]);
					};
					if (mod.indexOf("stealth") > -1) {
						update_skills(["stealth"]);
					};
					if (mod.indexOf("technology") > -1) {
						update_skills(["technology"]);
					};
					if (mod.indexOf("tolerance") > -1) {
						update_skills(["tolerance"]);
					};
					if (mod.indexOf("vice") > -1) {
						update_skills(["vice"]);
					};
					if (mod.indexOf("warfare") > -1) {
						update_skills(["warfare"]);
					};
				});
			};
			...
			var update_attack_from_item = function(itemid, attackid, options) {
				getAttrs(["repeating_inventory_" + itemid + "_itemname", "repeating_inventory_" + itemid + "_itemproperties", "repeating_inventory_" + itemid + "_itemmodifiers", "might", "agility"], function(v) {
					var update = {};
					var itemtype;
					var damage;
					var damagetype;
					...
					if ((itemtype && itemtype.indexOf("Ranged") > -1) || (finesse && +v.agility > +v.might)) {
						update["repeating_attack_" + attackid + "_atkattr_base"] = "@{agility_mod}";
						update["repeating_attack_" + attackid + "_dmgattr"] = "@{agility_mod}";
					} else {
						update["repeating_attack_" + attackid + "_atkattr_base"] = "@{might_mod}";
						update["repeating_attack_" + attackid + "_dmgattr"] = "@{might_mod}";
					}
			...
			var update_attacks = function(update_id, source) {
				...
				} else if (["might", "agility", "resolve", "intellect", "focus", "charisma", "devotion", "fortune", "spells", "all"].indexOf(update_id) > -1) {
			...
			var do_update_attack = function(attack_array, source) {
				var attack_attribs = ["level", "d20", "pb", "pb_type", "pbd_safe", "dtype", "globalmagicmod", "might_mod", "agility_mod", "resolve_mod", "intellect_mod", "focus_mod", "charisma_mod", "devotion_mod", "fortune_mod", "spellcasting_ability", "spell_save_dc", "spell_attack_mod", "spell_dc_mod", "global_damage_mod_roll", "global_damage_mod_crit"];
			...
			var update_weight = function() {
				...
				var weight_attrs = ["cp", "sp", "ep", "gp", "pp", "currency_value", "encumberance_setting", "might", "might_mod", "size", "carrying_capacity_mod", "inventory_slots_mod", "use_inventory_slots", "itemweightfixed", "itemslotsfixed"];
				getSectionIDs("repeating_inventory", function(idarray) {
					_.each(idarray, function(currentID, i) {
			...
			var update_ac = function() {
				getAttrs(["custom_ac_flag"], function(v) {
					if (v.custom_ac_flag === "2") {
						...
						var update = {};
						var ac_attrs = ["simpleinventory", "custom_ac_base", "custom_ac_part1", "custom_ac_part2", "might_mod", "agility_mod", "resolve_mod", "intellect_mod", "focus_mod", "charisma_mod", "devotion_mod", "fortune_mod", "custom_ac_shield"];
			...
			var update_class = function() {
				getAttrs(["class", "base_level", "custom_class", "cust_classname", "cust_hitdietype", "cust_spellcasting_ability", "cust_might_save_prof", "cust_agility_save_prof", "cust_resolve_save_prof", "cust_intellect_save_prof", "cust_focus_save_prof", "cust_charisma_save_prof", "cust_devvotion_save_prof", "cust_fortune_save_prof", "might_save_prof", "agility_save_prof", "resolve_save_prof", "intellect_save_prof", "focus_save_prof", "charisma_save_prof", "devotion_save_prof", "fortune_save_prof", "subclass", "multiclass1", "multiclass1_subclass", "multiclass2", "multiclass2_subclass", "multiclass3", "multiclass3_subclass", "npc"], function(v) {
					if (v.npc && v.npc == "1") {
						return;
					}
					if (v.custom_class && v.custom_class != "0") {
						setAttrs({
							...
							might_save_prof: v.cust_might_save_prof,
							agility_save_prof: v.cust_agility_save_prof,
							resolve_save_prof: v.cust_resolve_save_prof,
							intellect_save_prof: v.cust_intellect_save_prof,
							focus_save_prof: v.cust_focus_save_prof,
							charisma_save_prof: v.cust_charisma_save_prof,
							devotion_save_prof: v.cust_devotion_save_prof,
							fortune_save_prof: v.cust_fortune_save_prof
						});
					} else {
						update = {};
						//needs additional updating to reflect new classes
						switch (v.class) {
							case "":
								update["hitdietype"] = 6;
								update["spellcasting_ability"] = "0*";
								update["might_save_prof"] = 0;
								update["agility_save_prof"] = 0;
								update["resolve_save_prof"] = 0;
								update["intellect_save_prof"] = 0;
								update["focus_save_prof"] = 0;
								update["charisma_save_prof"] = 0;
								update["devotion_save_prof"] = 0;
								update["fortune_save_prof"] = 0;
								break;
							case "Alchemist":
								update["hitdietype"] = 6;
								update["spellcasting_ability"] = "0*";
			...
			//needs updating for new classes
			...
			var update_spell_info = function(attr) {
				var update = {};
				getAttrs(["spellcasting_ability", "spell_dc_mod", "globalmagicmod", "might_mod", "agility_mod", "resolve_mod", "intellect_mod", "focus_mod", "charisma_mod", "devotion_mod", "fortune_mod"], function(v) {
			...
			var update_passive_perception = function() {
				getAttrs(["pb_type", "passiveperceptionmod", "perception_bonus"], function(v) {
					...
					setAttrs({
						passive_focus: passive_perception
					})
				});
			};

			var update_kine_display = function() {
				getAttrs(["kine", "subkine"], function(v) {
					var final_kine = "";
					final_kine = v.subkine ? v.subkine : v.kine;
					if (v.kine.toLowerCase() === "dragonborn") {
						final_kine = v.kine;
					};
					setAttrs({
						kine_display: final_kine
					});
				});
			};
			...
			var update_npc_saves = function() {
				getAttrs(["npc_mig_save_base", "npc_agi_save_base", "npc_res_save_base", "npc_int_save_base", "npc_foc_save_base", "npc_cha_save_base", "npc_dev_save_base", "npc_for_save_base"], function(v) {
					var update = {};
					var last_save = 0;
					var for_save_flag = 0;
					var for_save = "";
					...
					var agi_save_flag = 0;
					var agi_save = "";
					var might_save_flag = 0;
					var might_save = "";
					// 1 = Positive, 2 = Last, 3 = Negative, 4 = Last Negative
					if (v.npc_for_save_base && v.npc_for_save_base != "@{fortune_mod}") {
						for_save = parseInt(v.npc_for_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							for_save_flag = for_save < 0 ? 4 : 2;
						} else {
							for_save_flag = for_save < 0 ? 3 : 1;
						}
					} else {
						for_save_flag = 0;
						for_save = "";
					};
					if (v.npc_dev_save_base && v.npc_dev_save_base != "@{devotion_mod}") {
						dev_save = parseInt(v.npc_dev_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							dev_save_flag = dev_save < 0 ? 4 : 2;
						} else {
							dev_save_flag = dev_save < 0 ? 3 : 1;
						}
					} else {
						dev_save_flag = 0;
						dev_save = "";
					...
					if (v.npc_foc_save_base && v.npc_foc_save_base != "@{foc_mod}") {
						foc_save = parseInt(v.npc_foc_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							foc_save_flag = foc_save < 0 ? 4 : 2;
						} else {
							foc_save_flag = foc_save < 0 ? 3 : 1;
						}
					} else {
						foc_save_flag = 0;
						foc_save = "";
					};
					if (v.npc_int_save_base && v.npc_int_save_base != "@{intellect_mod}") {
					...
					if (v.npc_res_save_base && v.npc_res_save_base != "@{resolve_mod}") {
						res_save = parseInt(v.npc_res_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							res_save_flag = res_save < 0 ? 4 : 2;
						} else {
							res_save_flag = res_save < 0 ? 3 : 1;
						}
					} else {
						res_save_flag = 0;
						res_save = "";
					};
					if (v.npc_agi_save_base && v.npc_agi_save_base != "@{agility_mod}") {
						agi_save = parseInt(v.npc_agi_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							agi_save_flag = agi_save < 0 ? 4 : 2;
						} else {
							agi_save_flag = agi_save < 0 ? 3 : 1;
						}
					} else {
						agi_save_flag = 0;
						agi_save = "";
					};
					if (v.npc_mig_save_base && v.npc_mig_save_base != "@{might_mod}") {
						mig_save = parseInt(v.npc_mig_save_base, 10);
						if (last_save === 0) {
							last_save = 1;
							mig_save_flag = mig_save < 0 ? 4 : 2;
						} else {
							mig_save_flag = mig_save < 0 ? 3 : 1;
						}
					} else {
						mig_save_flag = 0;
						mig_save = "";
					...
					update["npc_saving_flag"] = "" + for_save + dev_save + cha_save + foc_save + int_save + res_save + agi_save + mig_save;
					update["npc_mig_save"] = mig_save;
					update["npc_mig_save_flag"] = mig_save_flag;
					update["npc_agi_save"] = agi_save;
					update["npc_agi_save_flag"] = agi_save_flag;
					update["npc_res_save"] = res_save;
					update["npc_res_save_flag"] = res_save_flag;
					update["npc_int_save"] = int_save;
					update["npc_int_save_flag"] = int_save_flag;
					update["npc_foc_save"] = foc_save;
					update["npc_foc_save_flag"] = foc_save_flag;
					update["npc_cha_save"] = cha_save;
					update["npc_cha_save_flag"] = cha_save_flag;
					update["npc_dev_save"] = dev_save;
					update["npc_dev_save_flag"] = dev_save_flag;
					update["npc_for_save"] = for_save;
					update["npc_for_save_flag"] = for_save_flag;
					setAttrs(update, {
						silent: true
					});
				});
			...
			var update_npc_skills = function() {
				getAttrs(["npc_acrobatics_base", "npc_adamance_base", "npc_arcana_base", "npc_athletics_base", "npc_civics_base", "npc_culture_base", "npc_deception_base", "npc_deduction_base", "npc_domestication_base", "npc_endurance_base", "npc_exploration_base", "npc_grip_base", "npc_history_base", "npc_insight_base", "npc_intimidation_base", "npc_legerdemain_base", "npc_mathematics_base", "npc_medicine_base", "npc_nature_base", "npc_perception_base", "npc_performance_base", "npc_persuasion_base", "npc_religion_base", "npc_science_base", "npc_stealth_base", "npc_technology_base", "npc_tolerance_base", "npc_vice_base", "npc_warfare_base"], function(v) {
					var update = {};
					var last_skill = 0;
					var warfare_flag = 0;
					var warfare = "";
					var vice_flag = 0;
					var vice = "";
					var tolerance_flag = 0;
					var tolerance = "";
					var technology_flag = 0;
					var technology = "";
					var stealth_flag = 0;
					var stealth = "";
					var science_flag = 0;
					...
					var medicine_flag = 0;
					var medicine = "";
					var mathematics_flag = 0;
					var mathematics = "";
					var legerdemain_flag = 0;
					var legerdemain = "";
					...
					var grip_flag = 0;
					var grip = "";
					var exploration_flag = 0;
					var exploration = "";
					var endurance_flag = 0;
					var endurance = "";
					var domestication_flag = 0;
					var domestication = "";
					var deduction_flag = 0;
					var deduction = "";
					...
					// 1 = Positive, 2 = Last, 3 = Negative, 4 = Last Negative
					if (v.npc_warfare_base && v.npc_warfare_base != "@{intellect_mod}") {
						warfare = parseInt(v.npc_warfare_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							warfare_flag = warfare < 0 ? 4 : 2;
						} else {
							warfare_flag = warfare < 0 ? 3 : 1;
						}
					} else {
						warfare_flag = 0;
						warfare = "";
					};
					if (v.npc_vice_base && v.npc_vice_base != "@{intellect_mod}") {
						vice = parseInt(v.npc_vice_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							vice_flag = vice < 0 ? 4 : 2;
						} else {
							vice_flag = vice < 0 ? 3 : 1;
						}
					} else {
						vice_flag = 0;
						vice = "";
					};
					if (v.npc_tolerance_base && v.npc_tolerance_base != "@{resolve_mod}") {
						tolerance = parseInt(v.npc_tolerance_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							tolerance_flag = tolerance < 0 ? 4 : 2;
						} else {
							tolerance_flag = tolerance < 0 ? 3 : 1;
						}
					} else {
						tolerance_flag = 0;
						tolerance = "";
					};
					if (v.npc_technology_base && v.npc_technology_base != "@{intellect_mod}") {
						technology = parseInt(v.npc_technology_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							technology_flag = technology < 0 ? 4 : 2;
						} else {
							technology_flag = technology < 0 ? 3 : 1;
						}
					} else {
						technology_flag = 0;
						technology = "";
					};
					if (v.npc_stealth_base && v.npc_stealth_base != "@{agility_mod}") {
					...
					if (v.npc_science_base && v.npc_science_base != "@{intellect_mod}") {
						science = parseInt(v.npc_science_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							science_flag = science < 0 ? 4 : 2;
						} else {
							science_flag = science < 0 ? 3 : 1;
						}
					} else {
						science_flag = 0;
						science = "";
					};
					if (v.npc_religion_base && v.npc_religion_base != "@{intellect_mod}") {
					...
					if (v.npc_performance_base && v.npc_performance_base != "@{charisma_mod}") {
						performance = parseInt(v.npc_performance_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							performance_flag = performance < 0 ? 4 : 2;
						} else {
							performance_flag = performance < 0 ? 3 : 1;
						}
					} else {
						performance_flag = 0;
						performance = "";
					};
					if (v.npc_perception_base && v.npc_perception_base != "@{focus_mod}") {
					...
					if (v.npc_nature_base && v.npc_nature_base != "@{intellect_mod}") {
					...
					if (v.npc_medicine_base && v.npc_medicine_base != "@{intellect_mod}") {
					...
					if (v.npc_mathematics_base && v.npc_mathematics_base != "@{intellect_mod}") {
						mathematics = parseInt(v.npc_mathematics_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							mathematics_flag = mathematics < 0 ? 4 : 2;
						} else {
							mathematics_flag = mathematics < 0 ? 3 : 1;
						}
					} else {
						mathematics_flag = 0;
						mathematics = "";
					};
					if (v.npc_legerdemain_base && v.npc_legerdemain_base != "@{agility_mod}") {
						legerdemain = parseInt(v.npc_legerdemain_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							legerdemain_flag = legerdemain < 0 ? 4 : 2;
						} else {
							legerdemain_flag = legerdemain < 0 ? 3 : 1;
						}
					} else {
						legerdemain_flag = 0;
						legerdemain = "";
					...
					if (v.npc_insight_base && v.npc_insight_base != "@{focus_mod}") {
					...
					if (v.npc_history_base && v.npc_history_base != "@{intellect_mod}") {
					...
					if (v.npc_grip_base && v.npc_grip_base != "@{might_mod}") {
						grip = parseInt(v.npc_grip_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							grip_flag = grip < 0 ? 4 : 2;
						} else {
							grip_flag = grip < 0 ? 3 : 1;
						}
					} else {
						grip_flag = 0;
						grip = "";
					};
					if (v.npc_exploration_base && v.npc_exploration_base != "@{foc_mod}") {
						exploration = parseInt(v.npc_exploration_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							exploration_flag = exploration < 0 ? 4 : 2;
						} else {
							exploration_flag = exploration < 0 ? 3 : 1;
						}
					} else {
						exploration_flag = 0;
						exploration = "";
					};
					if (v.npc_endurance_base && v.npc_endurance_base != "@{resolve_mod}") {
						endurance = parseInt(v.npc_endurance_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							endurance_flag = endurance < 0 ? 4 : 2;
						} else {
							endurance_flag = endurance < 0 ? 3 : 1;
						}
					} else {
						endurance_flag = 0;
						endurance = "";
					};
					if (v.npc_domestication_base && v.npc_domestication_base != "@{charisma_mod}") {
						domestication = parseInt(v.npc_domestication_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							domestication_flag = domestication < 0 ? 4 : 2;
						} else {
							domestication_flag = domestication < 0 ? 3 : 1;
						}
					} else {
						domestication_flag = 0;
						domestication = "";
					};
					if (v.npc_deduction_base && v.npc_deduction_base != "@{intellect_mod}") {
						deduction = parseInt(v.npc_deduction_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							deduction_flag = deduction < 0 ? 4 : 2;
						} else {
							deduction_flag = deduction < 0 ? 3 : 1;
						}
					} else {
						deduction_flag = 0;
						deduction = "";
					...
					if (v.npc_culture_base && v.npc_culture_base != "@{intellect_mod}") {
						culture = parseInt(v.npc_culture_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							culture_flag = culture < 0 ? 4 : 2;
						} else {
							culture_flag = culture < 0 ? 3 : 1;
						}
					} else {
						culture_flag = 0;
						culture = "";
					};
					if (v.npc_civics_base && v.npc_civics_base != "@{intellect_mod}") {
						civics = parseInt(v.npc_civics_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							civics_flag = civics < 0 ? 4 : 2;
						} else {
							civics_flag = civics < 0 ? 3 : 1;
						}
					} else {
						civics_flag = 0;
						civics = "";
					};
					if (v.npc_athletics_base && v.npc_athletics_base != "@{might_mod}") {
					...
					if (v.npc_arcana_base && v.npc_arcana_base != "@{intellect_mod}") {
					...
					if (v.npc_adamance_base && v.npc_adamance_base != "@{devotion_mod}") {
						adamance = parseInt(v.npc_adamance_base, 10);
						if (last_skill === 0) {
							last_skill = 1;
							adamance_flag = adamance < 0 ? 4 : 2;
						} else {
							adamance_flag = adamance < 0 ? 3 : 1;
						}
					} else {
						adamance_flag = 0;
						adamance = "";
					};
					if (v.npc_acrobatics_base && v.npc_acrobatics_base != "@{agility_mod}") {
					...
					update["npc_skills_flag"] = "" + acrobatics + adamance + arcana + athletics + civics + culture + deception + deduction + domestication + endurance + exploration + grip + history + insight + intimidation + legerdemain + mathematics + medicine + nature + perception + performance + persuasion + religion + science + stealth + technology + tolerance + vice + warfare;
					
					update["npc_acrobatics"] = acrobatics;
					update["npc_acrobatics_flag"] = acrobatics_flag;
					update["npc_adamance"] = adamance;
					update["npc_adamance_flag"] = adamance_flag;
					...
					update["npc_civics"] = civics;
					update["npc_civics_flag"] = civics_flag;
					update["npc_culture"] = culture;
					update["npc_culture_flag"] = culture_flag;
					update["npc_deception"] = deception;
					update["npc_deception_flag"] = deception_flag;
					update["npc_deduction"] = deduction;
					update["npc_deduction_flag"] = deduction_flag;
					update["npc_domestication"] = domestication;
					update["npc_domestication_flag"] = domestication_flag;
					update["npc_endurance"] = endurance;
					update["npc_endurance_flag"] = endurance_flag;
					update["npc_exploration"] = exploration;
					update["npc_exploration_flag"] = exploration_flag;
					update["npc_grip"] = grip;
					update["npc_grip_flag"] = grip_flag;
					...
					update["npc_legerdemain"] = legerdemain;
					update["npc_legerdemain_flag"] = legerdemain_flag;
					update["npc_mathematics"] = mathematics;
					update["npc_mathematics_flag"] = mathematics_flag;
					...
					update["npc_science"] = science;
					update["npc_science_flag"] = science_flag;
					...
					update["npc_tolerance"] = tolerance;
					update["npc_tolerance_flag"] = tolerance_flag;
					update["npc_vice"] = vice;
					update["npc_vice_flag"] = vice_flag;
					update["npc_warfare"] = warfare;
					update["npc_warfare_flag"] = warfare_flag;
					setAttrs(update, {
						silent: true
					});
				});
